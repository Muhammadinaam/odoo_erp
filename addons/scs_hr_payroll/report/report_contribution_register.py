...
class ContributionRegisterReport(models.AbstractModel):
    ...
    def _get_report_values(self, docids, data=None):
        ...
        data['form']['date_from'] = (datetime.strptime(data['form'].get('date_from', fields.Date.today()),'%Y-%m-%d')).strftime('%m-%d-%Y')
        data['form']['date_to'] = (datetime.strptime(data['form'].get('date_to', str(
            datetime.now() + relativedelta(months=+1, day=1, days=-1))[:10]),'%Y-%m-%d')).strftime('%m-%d-%Y')
        return {
